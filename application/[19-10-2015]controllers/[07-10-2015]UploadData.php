...
//        $data = $this->do_upload();
//        
//        $save = $this->saveDb($data);
//        
//        $arr_ins = array("upload_filename"=>$data['original_filename'],
//                         "upload_size"=>$data['size'],
//                         "upload_row"=>$save);
//        
//        $log_id = $this->Mod_upload->insert_log($arr_ins);
//        
//        delete_files('./uploads/');
//        
//        echo json_encode($arr_ins);
//    }
    ...
    private function insert_penyelesaian_waste($arr)
    {
        ...
        foreach($arr as $arrS)
        {
            if($i>=6)
            {
                ...
                $arrD['gudang']             = trim($arrS['I']);
                $arrD['nilai']              = trim($arrS['J']);
                $arrD['material_document']  = trim($arrS['K']);
                
                $crc32                      = sprintf("%x", crc32(implode('', $arrD)));
                
                $rowCrc                     = $this->Mod_penyelesaianwaste->select_master(array('crc'=>$crc32))->num_rows();
