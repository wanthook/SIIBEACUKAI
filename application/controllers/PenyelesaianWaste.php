    ...
    function table()
    {
        ...
        foreach($dMaRes as $res)
        {
            $data[] = array(
				...
				'tanggal_document'              => $this->fungsi->convertDate($res->tanggal_document,"d-m-Y"),
                'mark'                  => $res->mark,
...
//        echo $data->num_rows();
        ...
        if($data->num_rows()>0)
        {
            ...
            foreach ($res as $dataKey => $dataValue)
            {
				...
				$tabel .= '<td>'.$this->fungsi->convertDate($dataValue->tanggal_document,"d-m-Y").'</td>';
                $tabel .= '</tr>';
    ...
    {        
        ...
        if($data->num_rows()>0)
        {
            ...
            foreach ($res as $dataKey => $dataValue)
            {
                ...
                $table[]    = array(
                    ...
                    $dataValue->gudang,
                    $dataValue->nilai,
                    $dataValue->material_document,
					$this->fungsi->convertDate($dataValue->tanggal_document,"d-m-Y")
                );
                $no++;
            }
        }
        ...
        $ret    = array(
            'laporan'           => "Laporan Penyelesaian Waste",
            'tanggal_awal'      => $sD,
            'tanggal_akhir'     => $eD,
            'head'              => $head,
            'table'             => $table
