...
class Asset_ij extends Secure_area
{
    ...
    public function form_add()
    {
        ...
        echo $this->load->view('asset_ij_form', $data, TRUE);
    }
    ...
    public function form_add_detail()
    {
        ...
        echo $this->load->view('asset_ij_form_detail', $data, TRUE);
    }
...
//        echo $id;
        ...
        $this->load->view('asset_ij_detail',$view);
    }
    ...
    public function form_add_save()
    {
        ...
        {
            $arr['status'] = '1';
            $arr['message'] = $this->lang->line('asset_ij_master_success');
        }
        else
        {
            $arr['status'] = '0';
            $arr['message'] = $this->lang->line('asset_ij_master_failed');
        }
...
//        $data_ins['delivery_status']= $delivery_status;
        ...
        if(empty($detail_id))
        {
            ...
            {
                $arr['status'] = '1';
                $arr['message'] = $this->lang->line('asset_ij_detail_success');
            }
            else
            {
                $arr['status'] = '0';
                $arr['message'] = $this->lang->line('asset_ij_detail_failed');
            }
        }
    ...
    public function form_add_update()
    {
        ...
        {
            $arr['status'] = '0';
            $arr['message'] = $this->lang->line('asset_ij_master_noedit');
        }
    ...
    public function form_delete($id)
    {
        ...
        if(count($data_ins)>0)
        {
            ...
            $arr['status'] = '1';
            $arr['message'] = $this->lang->line('asset_ij_master_deleted');
        }
        else
        {
            $arr['status'] = '0';
            $arr['message'] = $this->lang->line('asset_ij_detail_noedit');
        }
...
//        }        
    ...
    public function form_delete_detail($id)
    {
        ...
        if(count($data_ins)>0)
        {
            ...
            $arr['status'] = '1';
            $arr['message'] = $this->lang->line('asset_ij_detail_deleted');
        }
        else
        {
            $arr['status'] = '0';
    ...
    private function create_table_master($data,$total_data,$offset_data)
    {
        ...
        else
        {
            ...
            {            
                ...
                {
                    $button = anchor($this->con_page.'/form_edit/'.$row->master_id,'<i class="fa fa-pencil"></i>',array('class'=>'btn btn-warning btn-xs btn-edit','title'=>$this->lang->line('asset_ij_button_edit')))."&nbsp;".
                              anchor($this->con_page.'/detail/'.$row->master_id,'<i class="fa fa-bars"></i>',array('class'=>'btn btn-primary btn-xs','title'=>$this->lang->line('asset_ij_button_detail'))).'&nbsp;'.
                              anchor($this->con_page.'/form_finish/'.$row->master_id,'<i class="fa fa-flag"></i>',array('class'=>'btn btn-success btn-xs btn-finish','title'=>$this->lang->line('asset_ij_button_finish')))."&nbsp;".
                              anchor($this->con_page.'/form_delete/'.$row->master_id,'<i class="fa fa-close"></i>',array('class'=>'btn btn-danger btn-xs btn-del','title'=>$this->lang->line('asset_ij_button_delete')));
                }
                else if(in_array($this->session->userdata('type'), array('IMPORT','FINANCE')))
                {
                    $button = 
//                              anchor($this->con_page.'/form_edit/'.$row->master_id,'<i class="fa fa-pencil"></i>',array('class'=>'btn btn-warning btn-xs btn-edit','title'=>$this->lang->line('asset_ij_button_edit')))."&nbsp;".
                              anchor($this->con_page.'/detail/'.$row->master_id,'<i class="fa fa-bars"></i>',array('class'=>'btn btn-primary btn-xs','title'=>$this->lang->line('asset_ij_button_detail')));
//                        anchor($this->con_page.'/finish/'.$row->master_id,'<i class="fa fa-flag"></i>',array('class'=>'btn btn-success btn-xs','title'=>$this->lang->line('asset_ij_button_finish')))."&nbsp;".
//                    anchor($this->con_page.'/delete/'.$row->master_id,'<i class="fa fa-close"></i>',array('class'=>'btn btn-danger btn-xs','title'=>$this->lang->line('asset_ij_button_delete')));
                }
...
//                              anchor($this->con_page.'/finish/'.$row->master_id,'<i class="fa fa-flag"></i>',array('class'=>'btn btn-success btn-xs','title'=>$this->lang->line('asset_ij_button_finish')))."&nbsp;".
                    ...
                    {
                        $button = anchor($this->con_page.'/detail/'.$row->master_id,'<i class="fa fa-bars"></i>',array('class'=>'btn btn-primary btn-xs','title'=>$this->lang->line('asset_ij_button_detail')));
//                              
                    }
                }
...
//                }
                ...
                if(in_array($this->session->userdata('type'), array('ADMIN')))
                {
                    $button = anchor($this->con_page.'/form_edit_detail/'.$row->detail_id,'<i class="fa fa-pencil"></i>',array('class'=>'btn btn-warning btn-xs btn-edit','title'=>$this->lang->line('asset_ij_button_edit')))."&nbsp;".
//                    anchor($this->con_page.'/detail/'.$row->master_id,'<i class="fa fa-bars"></i>',array('class'=>'btn btn-primary btn-xs','title'=>$this->lang->line('asset_ij_button_detail'))).'&nbsp;'.
                              anchor($this->con_page.'/form_finish_detail/'.$row->detail_id,'<i class="fa fa-flag"></i>',array('class'=>'btn btn-success btn-xs btn-finish','title'=>$this->lang->line('asset_ij_button_finish')))."&nbsp;".
                              anchor($this->con_page.'/form_delete_detail/'.$row->detail_id,'<i class="fa fa-close"></i>',array('class'=>'btn btn-danger btn-xs btn-del','title'=>$this->lang->line('asset_ij_button_delete')));
                }
                else if(in_array($this->session->userdata('type'), array('IMPORT','FINANCE')))
                {
                    $button = 
//                              anchor($this->con_page.'/form_add_update/'.$row->master_id,'<i class="fa fa-pencil"></i>',array('class'=>'btn btn-warning btn-xs','title'=>$this->lang->line('asset_ij_button_edit')))."&nbsp;".
                              anchor($this->con_page.'/form_edit_detail/'.$row->detail_id,'<i class="fa fa-pencil"></i>',array('class'=>'btn btn-warning btn-xs btn-edit','title'=>$this->lang->line('asset_ij_button_edit')));
//                        anchor($this->con_page.'/finish/'.$row->master_id,'<i class="fa fa-flag"></i>',array('class'=>'btn btn-success btn-xs','title'=>$this->lang->line('asset_ij_button_finish')))."&nbsp;".
//                    anchor($this->con_page.'/delete/'.$row->master_id,'<i class="fa fa-close"></i>',array('class'=>'btn btn-danger btn-xs','title'=>$this->lang->line('asset_ij_button_delete')));
...
//                              
                ...
                else if(in_array($this->session->userdata('type'), array('USER')))
                {
                    $button = anchor($this->con_page.'/form_edit_detail/'.$row->detail_id,'<i class="fa fa-pencil"></i>',array('class'=>'btn btn-warning btn-xs btn-edit','title'=>$this->lang->line('asset_ij_button_edit')))."&nbsp;".
//                    anchor($this->con_page.'/detail/'.$row->master_id,'<i class="fa fa-bars"></i>',array('class'=>'btn btn-primary btn-xs','title'=>$this->lang->line('asset_ij_button_detail'))).'&nbsp;'.
//                              anchor($this->con_page.'/form_finish_detail/'.$row->detail_id,'<i class="fa fa-flag"></i>',array('class'=>'btn btn-success btn-xs','title'=>$this->lang->line('asset_ij_button_finish')))."&nbsp;".
                              anchor($this->con_page.'/form_delete_detail/'.$row->detail_id,'<i class="fa fa-close"></i>',array('class'=>'btn btn-danger btn-xs btn-del','title'=>$this->lang->line('asset_ij_button_delete')));
