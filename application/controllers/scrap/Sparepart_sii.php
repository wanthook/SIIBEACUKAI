 ...
 */
/**
 * Description of Sparepart_sii
 *
 * @author wanthook
 */
class Sparepart_sii extends Secure_area
{
    
    private $table_tools            = "tblSparepartIj";
    private $table_tools_detail     = "tblSparepartDetailIj";
    private $con_page      = "Sparepart_sii";
    ...
    function __construct() 
    {
        ...
        $this->load->model('Mod_sparepart_ij');
    }
    ...
    public function index()
    {
        ...
        $data_count = $this->Mod_sparepart_ij->select_master($search)->num_rows();
        $data = $this->Mod_sparepart_ij->select_master($search,  $this->limit, $offset, $order)->result();
        
        $view['table'] = $this->create_table_master($data,$data_count,$offset);
        
        $this->load->view('sparepart_sii',$view);
    }
    ...
    public function form_add()
    {
        ...
        echo $this->load->view('sparepart_sii_form', $data, TRUE);
    }
    ...
    public function form_edit($id)
    {
        ...
        $res = $this->Mod_sparepart_ij->select_master(array('master_id'=>$id))->result();
    ...
    public function form_add_detail()
    {
        ...
        echo $this->load->view('sparepart_sii_form_detail', $data, TRUE);
    }
    ...
    public function form_edit_detail($id)
    {
        ...
        $res = $this->Mod_sparepart_ij->select_detail($search)->result();
    ...
    public function detail($id)
    {
        ...
        $data = $this->Mod_sparepart_ij->select_detail($search, 10000, 0, $order)->result();
        
        $master = $this->Mod_sparepart_ij->select_master(array('a.master_id'=>$id))->result();
//        echo $id;
        $view['table'] = $this->create_table_detail($data);
        $view['master'] = $master[0];
        
        $this->load->view('sparepart_sii_detail',$view);
    }
    ...
    public function form_add_save()
    {
        ...
        $ins = $this->Mod_sparepart_ij->create_master($data_ins);
        ...
        {
            $arr['status'] = '1';
            $arr['message'] = $this->lang->line('sparepart_sii_master_success');
        }
        else
        {
            $arr['status'] = '0';
            $arr['message'] = $this->lang->line('sparepart_sii_master_failed');
        }
    ...
    public function form_add_detail_save()
    {
        ...
        if(empty($detail_id))
        {
            $ins = $this->Mod_sparepart_ij->create_detail($data_ins);
            
            $data_upd['status'] = "PROCCESS";
            $data_upd['modified_by'] = $this->Mod_user->getSessionUserId();
            $data_upd['updated_at'] = date('Y-m-d H:i:s');
            
            $this->Mod_sparepart_ij->update_master($data_upd,$data_ins['master_id']);
        
            if($ins>0)
            {
                $arr['status'] = '1';
                $arr['message'] = $this->lang->line('sparepart_sii_detail_success');
            }
            else
            {
                $arr['status'] = '0';
                $arr['message'] = $this->lang->line('sparepart_sii_detail_failed');
            }
        }
    ...
    public function form_add_update()
    {
        ...
        if(count($data_ins)>0)
        {
            $this->Mod_sparepart_ij->update_master($data_ins,$master_id);
        ...
        {
            $arr['status'] = '0';
            $arr['message'] = $this->lang->line('sparepart_sii_master_noedit');
        }
    ...
    public function form_finish($id)
    {
        ...
        if(count($data_ins)>0)
        {
            $this->Mod_sparepart_ij->update_master($data_ins,$id);
    ...
    public function form_delete($id)
    {
        ...
        if(count($data_ins)>0)
        {
            ...
            $arr['status'] = '1';
            $arr['message'] = $this->lang->line('sparepart_sii_master_deleted');
        }
        else
        {
            $arr['status'] = '0';
            $arr['message'] = $this->lang->line('sparepart_sii_detail_noedit');
        }
    ...
    public function form_add_detail_update()
    {
         ...
         * 
         */
        $data_detail = $this->Mod_sparepart_ij->select_detail(array('detail_id'=>$detail_id))->result();
        /*
         * 
         */
        ...
        if(count($data_ins)>0)
        {
            $this->Mod_sparepart_ij->update_detail($data_ins,$detail_id);
    ...
    public function form_finish_detail($id)
    {
        ...
        if(count($data_ins)>0)
        {
            $this->Mod_sparepart_ij->update_detail($data_ins,$id);
    ...
    public function form_delete_detail($id)
    {
        ...
        if(count($data_ins)>0)
        {
            ...
            $arr['status'] = '1';
            $arr['message'] = $this->lang->line('sparepart_sii_detail_deleted');
        }
        else
        {
            $arr['status'] = '0';
    ...
    private function create_table_master($data,$total_data,$offset_data)
    {
        ...
        else
        {
            ...
            {            
                ...
                {
                    $button = anchor($this->con_page.'/form_edit/'.$row->master_id,'<i class="fa fa-pencil"></i>',array('class'=>'btn btn-warning btn-xs btn-edit','title'=>$this->lang->line('sparepart_sii_button_edit')))."&nbsp;".
                              anchor($this->con_page.'/detail/'.$row->master_id,'<i class="fa fa-bars"></i>',array('class'=>'btn btn-primary btn-xs','title'=>$this->lang->line('sparepart_sii_button_detail'))).'&nbsp;'.
                              anchor($this->con_page.'/form_finish/'.$row->master_id,'<i class="fa fa-flag"></i>',array('class'=>'btn btn-success btn-xs btn-finish','title'=>$this->lang->line('sparepart_sii_button_finish')))."&nbsp;".
                              anchor($this->con_page.'/form_delete/'.$row->master_id,'<i class="fa fa-close"></i>',array('class'=>'btn btn-danger btn-xs btn-del','title'=>$this->lang->line('sparepart_sii_button_delete')));
                }
                else if(in_array($this->session->userdata('type'), array('IMPORT')))
                {
                    $button = 
//                              anchor($this->con_page.'/form_edit/'.$row->master_id,'<i class="fa fa-pencil"></i>',array('class'=>'btn btn-warning btn-xs btn-edit','title'=>$this->lang->line('sparepart_sii_button_edit')))."&nbsp;".
                              anchor($this->con_page.'/detail/'.$row->master_id,'<i class="fa fa-bars"></i>',array('class'=>'btn btn-primary btn-xs','title'=>$this->lang->line('sparepart_sii_button_detail')));
//                        anchor($this->con_page.'/finish/'.$row->master_id,'<i class="fa fa-flag"></i>',array('class'=>'btn btn-success btn-xs','title'=>$this->lang->line('sparepart_sii_button_finish')))."&nbsp;".
//                    anchor($this->con_page.'/delete/'.$row->master_id,'<i class="fa fa-close"></i>',array('class'=>'btn btn-danger btn-xs','title'=>$this->lang->line('sparepart_sii_button_delete')));
                }
...
//                              anchor($this->con_page.'/finish/'.$row->master_id,'<i class="fa fa-flag"></i>',array('class'=>'btn btn-success btn-xs','title'=>$this->lang->line('sparepart_sii_button_finish')))."&nbsp;".
                    ...
                    {
                        $button = anchor($this->con_page.'/detail/'.$row->master_id,'<i class="fa fa-bars"></i>',array('class'=>'btn btn-primary btn-xs','title'=>$this->lang->line('sparepart_sii_button_detail')));
//                              
                    }
                }
...
//            $i = 1 + $offset_data;
            ...
            {   
                ...
                if(in_array($this->session->userdata('type'), array('ADMIN')))
                {
                    $button = anchor($this->con_page.'/form_edit_detail/'.$row->detail_id,'<i class="fa fa-pencil"></i>',array('class'=>'btn btn-warning btn-xs btn-edit','title'=>$this->lang->line('sparepart_sii_button_edit')))."&nbsp;".
//                    anchor($this->con_page.'/detail/'.$row->master_id,'<i class="fa fa-bars"></i>',array('class'=>'btn btn-primary btn-xs','title'=>$this->lang->line('sparepart_sii_button_detail'))).'&nbsp;'.
                              anchor($this->con_page.'/form_finish_detail/'.$row->detail_id,'<i class="fa fa-flag"></i>',array('class'=>'btn btn-success btn-xs btn-finish','title'=>$this->lang->line('sparepart_sii_button_finish')))."&nbsp;".
                              anchor($this->con_page.'/form_delete_detail/'.$row->detail_id,'<i class="fa fa-close"></i>',array('class'=>'btn btn-danger btn-xs btn-del','title'=>$this->lang->line('sparepart_sii_button_delete')));
                }
                else if(in_array($this->session->userdata('type'), array('IMPORT')))
                {
                    $button = 
//                              anchor($this->con_page.'/form_add_update/'.$row->master_id,'<i class="fa fa-pencil"></i>',array('class'=>'btn btn-warning btn-xs','title'=>$this->lang->line('sparepart_sii_button_edit')))."&nbsp;".
                              anchor($this->con_page.'/form_edit_detail/'.$row->detail_id,'<i class="fa fa-pencil"></i>',array('class'=>'btn btn-warning btn-xs btn-edit','title'=>$this->lang->line('sparepart_sii_button_edit')));
//                        anchor($this->con_page.'/finish/'.$row->master_id,'<i class="fa fa-flag"></i>',array('class'=>'btn btn-success btn-xs','title'=>$this->lang->line('sparepart_sii_button_finish')))."&nbsp;".
//                    anchor($this->con_page.'/delete/'.$row->master_id,'<i class="fa fa-close"></i>',array('class'=>'btn btn-danger btn-xs','title'=>$this->lang->line('sparepart_sii_button_delete')));
...
//                              
                ...
                else if(in_array($this->session->userdata('type'), array('USER')))
                {
                    $button = anchor($this->con_page.'/form_edit_detail/'.$row->detail_id,'<i class="fa fa-pencil"></i>',array('class'=>'btn btn-warning btn-xs btn-edit','title'=>$this->lang->line('sparepart_sii_button_edit')))."&nbsp;".
//                    anchor($this->con_page.'/detail/'.$row->master_id,'<i class="fa fa-bars"></i>',array('class'=>'btn btn-primary btn-xs','title'=>$this->lang->line('sparepart_sii_button_detail'))).'&nbsp;'.
//                              anchor($this->con_page.'/form_finish_detail/'.$row->detail_id,'<i class="fa fa-flag"></i>',array('class'=>'btn btn-success btn-xs','title'=>$this->lang->line('sparepart_sii_button_finish')))."&nbsp;".
                              anchor($this->con_page.'/form_delete_detail/'.$row->detail_id,'<i class="fa fa-close"></i>',array('class'=>'btn btn-danger btn-xs btn-del','title'=>$this->lang->line('sparepart_sii_button_delete')));
...
//                $i++;
            ...
            $this->table->add_row(
                    array('data' => "<b>Total USD</b>", 'colspan' => 10,'style'=>'text-align: right;'),
                    array('data' => $total, 'colspan' => 8,'style'=>'text-align: left;')
                    );
        }
