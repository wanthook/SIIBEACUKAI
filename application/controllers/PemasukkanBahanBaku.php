...
//        echo $data->num_rows();
        ...
        $tabel .= '<tr>      
                   ...
                   <th rowspan="2">Jumlah</th>                   <th rowspan="2">Satuan</th>				   				   <th rowspan="2">Jumlah (LBS)</th>                   <th rowspan="2">Satuan (LBS)</th>
                   <th rowspan="2">Qty PIB</th>
                   <th rowspan="2">Amount PIB</th>
                   <th rowspan="2">Loss Qty</th>
        ...
        if($data->num_rows()>0)
        {
            ...
            foreach ($res as $dataKey => $dataValue)
            {
                ...
                $tabel .= '<td>'.$dataValue->jumlah_lbs.'</td>';                
				$tabel .= '<td>LBS</td>';
				//$tabel .= '<td>'.$dataValue->sQtyPib.'</td>';
                //$tabel .= '<td>'.$dataValue->sAmtPib.'</td>';
                //$tabel .= '<td>'.$dataValue->sQtyDn.'</td>';
...
//        $data    = $this->Mod_pemasukanbahanbaku->select_master($sel,0,0,array('nomor','asc'));
        ...
        $head    = array('No.',
                        'Jenis Dokumen',
                        'Nomor Dokumen Pabeanan',
                        'Tanggal Dokumen Pabeanan',
                        'No. Seri Barang Dokumen Pabeanan',				   
        ...
        if($data->num_rows()>0)
        {
            ...
            foreach ($res as $dataKey => $dataValue)
            {
                ...
                $table[]    = array(
                    $no,
                    $dataValue->jenis_dokumen,
                    $dataValue->nomor,
                    $this->fungsi->convertDate($dataValue->tanggal,"d-m-Y"),
                    $dataValue->nomor_seri,
                    $dataValue->nomor_bukti,
                    $this->fungsi->convertDate($dataValue->tanggal_bukti,"d-m-Y"),
                    $dataValue->material_code,
                    $dataValue->material_desc,
                    $dataValue->batch,
                    $dataValue->jumlah,                    $dataValue->satuan,					$dataValue->jumlah_lbs,                    'LBS',
                    $dataValue->qty_pib,
                    $dataValue->amount_pib,
                    $dataValue->qty_dn,    
                    $dataValue->mata_uang,
        ...
        $ret    = array(
            'laporan'           => "Laporan Pemasukkan Bahan Baku",
            'tanggal_awal'      => $sD,
            'tanggal_akhir'     => $eD,
            'head'              => $head,
            'table'             => $table
