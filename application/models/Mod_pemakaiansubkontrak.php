 ...
 * @author wanthook
 */
class Mod_pemakaiansubkontrak extends CI_Model
{
    private $table_user     = "user";
    
    private $table_master   = "pemakaiansubkontrak";
...
//    private $table_detail   = "bpbdetail";
                                  ...
                                  $select="")
    {
		...
		$this->db->select('SUM(distinct disubkontrak) as disubkontrak',false);
                $this->db->select('SUM(distinct disubkontrak_lbs) as disubkontrak_lbs',false);
...
//		$this->db->select('a.*',false);
        ...
        $this->db->select('SUM(distinct a.disubkontrak_lbs) as disubkontrak_lbs',false);
        $this->db->from($this->table_master." a");
        $this->db->join("pemasukanbahanbaku tbla","a.batch=tbla.batch",'LEFT');
        $this->db->group_by("a.nomor");
        $this->db->group_by("a.tanggal");
        ...
        foreach($row as $res)
        {
                $arrins[] = array(
                        ...
                        "batch" => $res->batch,
                        "jumlah" => $res->disubkontrak,
                        "satuan" => $res->satuan,
                        "jumlah_lbs" => $res->disubkontrak_lbs,
                        "gudang" => $res->gudang
                );
        }
